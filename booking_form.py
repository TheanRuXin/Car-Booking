...
def get_selected_car_details(car_id):
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("SELECT make_and_model, ROUND(daily_rate,2), seating_capacity, car_type, transmission_type, image_path FROM cars_details WHERE id = ?", (car_id,))
    car = cursor.fetchone()
    conn.close()
    return car
...
def fetch_discount_for_user():
    try:
        ...
        conn = sqlite3.connect(r"C:\Users\User\Documents\Ruxin file\build\Car_Rental.db")  # Assuming your promotions table is in 'Users.db'
        cursor = conn.cursor()

        # Fetch promotion details
        cursor.execute("SELECT name, discount_percentage FROM promotions WHERE promotion_id = 2")
...
def create_booking_page(car_id,user_id):
    ...
    # Check if any of the fields are empty
        ...
        try:
            ...
            cursor.execute("""
                            INSERT INTO history (car_id, user_id, customer_name, email, contact_number, date_of_birth, rental_start_date, rental_end_date, promotion, total_price)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """, (
                car_id,
                user_id,
